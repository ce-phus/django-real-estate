...
"""
    ...
    def get_all_agents(request):
        agents= Profile.objects.filter(is-agent=True)
        serializer=ProfileSerializer(agents, many=True)
        name_spaced_response={"agents": serializer.data}
        return Response(name_paced_response, status=status.HTTP_200_OK)
"""

class TopAgentssListAPIView(generics.ListAPIView):
    permission_classes=[permissions.IsAuthenticated]
